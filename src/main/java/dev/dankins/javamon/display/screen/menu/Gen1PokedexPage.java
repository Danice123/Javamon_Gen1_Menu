...
public class Gen1PokedexPage implements PokedexPageMenu {
	...
	public void init(final AssetManager assets, final RenderInfo ri) {
		...
		image = new ImageContent(
				assets.get("pokemon/" + pokemon.getNumber() + ".png", Texture.class)).alignBottom();
		number = new TextContent(font, "No." + pokemon.getFormattedNumber());
		stats = new VertBox(100, 15).addContent(new TextContent(font, pokemon.getName()))
				.addContent(new TextContent(font, pokemon.getSpecies())).addContent(() -> {
					final BasicBoxContent box = new HorzBox(0, 0).setSpacing(48)
							.addContent(new TextContent(font, "HT"));
					if (isCaught) {
						box.addContent(
								new TextContent(font, Integer.toString(pokemon.getHeight()) + "m"));
					} else {
					...
					final BasicBoxContent box = new HorzBox(0, 0).setSpacing(48)
							.addContent(new TextContent(font, "WT"));

					if (isCaught) {
						box.addContent(new TextContent(font,
								Integer.toString(pokemon.getWeight()) + "lb"));
					} else {
